...
template = """
 You are a marketing copywriter with 20 years of experience. You are analyzing customer's background to write personalized product description that only this customer will receive; 
    ...
    USE CASE: write a story in 5 sentences, of an example weekend activity taking into account serviceneed {serviceneed} and age {agegroup}, write a story in first person, example "I started my Saturday morning with ...";OUTPUT TEXT in Estonian;
"""

prompt = PromptTemplate(
    input_variables=["agegroup", "serviceneed", "content"],
    template=template,
)
...
with col2:
    st.image(image='companylogo.jpg', caption='Service for your needs')
...
def get_serviceneed():
    input_text = st.text_input(label="Customers service need", key="serviceneed_input")
    return input_text

serviceneed_input = get_serviceneed()
...
def update_text_with_example():
    print ("in updated")
    st.session_state.content_input = "transport services for your needs, best price, good customer service"
...
#        st.stop()
    ...
    prompt_with_content = prompt.format(agegroup=option_agegroup, serviceneed=serviceneed_input, content=content_input)
